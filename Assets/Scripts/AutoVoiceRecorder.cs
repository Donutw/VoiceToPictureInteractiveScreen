...
public class AutoVoiceRecorder : MonoBehaviour
{
    ...
    [Header("灵敏度设置")]
    public float silenceThreshold = 0.01f;     // 小于这个值视为安静
    ...
    void Start()
    {
        ...
        if (Microphone.devices.Length > 0)
        {
            if (string.IsNullOrEmpty(selectedMic))
            {
                ...
                Debug.Log("使用默认麦克风: " + selectedMic);
            }
        }
        else
        {
            Debug.LogWarning("没有检测到麦克风设备！");
            return;
        }
            ...
            if (!isRecording)
            {
                ...
                if (loudCheckTimer >= loudCheckPeriod)
                {
                    ...
                    if (validVolumes.Count > 0)
                    {
                        ...
                        Debug.Log($"🧪 有效帧数: {validVolumes.Count}, 高于阈值帧数: {loudCount}, 占比: {ratio:P0}");

                        if (ratio >= requiredLoudRatio)
                        {
                            Debug.Log("✅ 检测到足够的说话帧，开始录音！");
                            StartActualRecording();
                        }
                        else
                        {
                            Debug.Log("🟡 没有达到说话比例要求，继续监听");
                        }
                    }
                    else
                    {
                        Debug.Log("🔇 没有有效音量帧，跳过判断");
                    }
            ...
            else
            {
                ...
                if (silenceCheckTimer >= silenceCheckPeriod)
                {
                    ...
                    if (validVolumes.Count > 0)
                    {
                        ...
                        Debug.Log($"🧪 录音中检测：静音比例 {ratio:P0}");

                        if (ratio >= requiredSilenceRatio && totalRecordTime > 1.0f)
                        {
                            Debug.Log("🛑 检测到说话停止，保存录音");
                            StopAndSaveRecording();
                        }
                    }
    ...
    public float GetMicVolume()
    {
        ...
        {
            Debug.LogWarning("❗ AudioClip 为 null，无法读取音频数据");
            return 0f;
        }
        ...
        // 可选：调试打印（建议测试时开）
        //Debug.Log($"📈 音量: {rms:F5}（采样数: {length}）");
    ...
    void StopAndSaveRecording()
    {
        ...
        Debug.Log("✅ 音频已保存：" + path);

        StartMic(); // 保存后进入下一轮监听
    }

    // （可选）你可以在这里触发调用 Python whisper
}
